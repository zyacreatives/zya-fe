...
import { ProjectCreationSuccessDialog } from "../_components/project-creation-success-dialog";

export function Onboarding_CreativePortfolio() {
  const { form, isSuccess, onSubmit } = useSubmitProject("creative");
  const { cachedDisciplines, disciplines } = useGetCreativeDisciplinesApi();

  return (
    <div className=" flex w-full justify-evenly pb-12 mx-auto">
      <ProjectCreationSuccessDialog
        isSuccess={isSuccess}
        projectName={form.getValues("title") ?? ""}
      />
      <Form {...form}>
        <form
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <MultiSelectCommand
                      field={field}
                      options={disciplines ?? cachedDisciplines ?? []}
                    />
                  </FormControl>
                </FormItem>
              )}
